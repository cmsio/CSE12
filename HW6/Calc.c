...
long Calculator :: InToPost (void) {
	...
	while ((character = cin.get ()) != '\n') {
		...
		if (isdigit (character) || character == '.') {
			...
			stack1.Push (new Word (namein (buffer)));
		}
			...
			while (word = (Word *) stack2.Pop (), word->op->ascii != '(')
				stack1.Push (word);
		...
		/* operator must be encountered */
			...
			while (!stack2.IsEmpty () &&
				((Word *) stack2.Top ())->op->priority >= word->op->priority)
					stack1.Push (stack2.Pop ());
...
static long exp (long power, long x) {
	long retval = 1;
...
static long fact (long x, long ignored) {
	return (x <= 1) ? 1 : x * fact (x - 1, ignored);
}
